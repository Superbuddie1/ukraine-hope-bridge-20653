...
import {
  AlertTriangle, Wallet, Stethoscope, Heart, Users, MapPin,
  ClipboardList, ExternalLink, ChevronRight,
} from "lucide-react";
...
const iconMap: Record<string, React.ComponentType<{ className?: string }>> = {
  AlertTriangle, Wallet, Stethoscope, Heart, Users, ClipboardList,
};
...
const RoadmapSectionComponent = ({ section, index }: RoadmapSectionProps) => {
  ...
  return (
    <div className="relative">
      {/* Milestone circle – Desktop */}
      <div className="absolute left-1/2 top-0 -translate-x-1/2 -translate-y-4 z-20 hidden md:block">
        <div className="relative">
          <div className={`w-14 h-14 rounded-full flex items-center justify-center shadow-[var(--shadow-soft)] border-4 border-background transition-transform hover:scale-110 ${isEven ? 'bg-primary' : 'bg-teal'}`}>
        ...
        <div className="rounded-2xl border border-border/50 bg-card shadow-[var(--shadow-soft)] hover:shadow-[var(--shadow-medium)] transition-all duration-300 overflow-hidden">
          ...
          <div className="px-6 py-5 border-b border-border/40">
            <div className="md:hidden flex items-center gap-3 mb-3">
              ...
              <span className="text-xs font-semibold text-muted-foreground tracking-wide uppercase">{t('resources.step')} {index + 1}</span>
            </div>

            <h2 className="text-lg font-bold text-foreground leading-snug">
              {isUk && section.titleUa ? section.titleUa : section.title}
            </h2>
            {!isUk && section.titleUa && (
            ...
            {section.steps && section.steps.length > 0 && (
              <div className="space-y-3">
                {section.steps.map((step, idx) => (
                  <div key={step.id} className="group/step flex items-start gap-3 p-4 rounded-xl bg-secondary/50 border border-border/30 hover:border-primary/30 hover:bg-primary/[0.03] transition-all duration-200 cursor-default">
                    <div className={`shrink-0 w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold ${isEven ? 'bg-primary/10 text-primary' : 'bg-teal/10 text-teal'}`}>
                      {idx + 1}
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      {step.link && (
                        <Button variant="link" className="h-auto p-0 mt-2 text-primary text-sm" onClick={() => window.open(step.link, '_blank')}>
                          {step.linkText || t('resources.learnMore')}
            ...
            {section.recommendations.length > 0 && (
              <div className="space-y-3">
                {section.recommendations.map((rec, idx) => (
                  <RoadmapCard key={rec.resource.id} recommendation={rec} index={idx} variant={isEven ? 'blue' : 'teal'} />
                ))}
              </div>
            )}
          </div>
